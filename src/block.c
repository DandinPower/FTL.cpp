...
int ProgramBlock(Block *block, int lbas[], int lbaNums, BlockType_t type) {
    if (block->type == NONE) block->type = type;
    if (block->type != type) printf("Program on different block type\n");
    if (block->currentPageIndex == PAGE_IN_BLOCK_NUM) printf("Insufficent space in block to program\n");
    if (block->pages[block->currentPageIndex].status != FREE) printf("Program on not free page\n");
